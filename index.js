...
CASAuthentication.prototype._login = function(req, res, next) {
  ...
  // only include the renew parameter when truthy
 ...
 * Logout the currently logged in CAS user.
 */
CASAuthentication.prototype.handle_single_logout = function(req, res) {
  ...
  var xml = req.body.logoutRequest;
  ...
  parseXML(xml, {
    ...
    explicitArray: false,
    tagNameProcessors: [XMLprocessors.normalize, XMLprocessors.stripPrefix]
  }, function(err, parseResult) {
    if (err) {
      throw new Error('Response from CAS server was bad.');
    }
    var st = parseResult.logoutrequest.sessionindex;

    req.sessionStore.get(st, function(err, sessionResult) {
      if (err) {
      ...
      if (sessionResult && sessionResult.sid) {
        console.log('Find sid by st succeed, trying to destroy it.', 'st', st, 'sessionId', sessionResult.sid);
        req.sessionStore.destroy(sessionResult.sid, function(err) {
          /* istanbul ignore if */
          if (err) {
            console.log('Error when destroy session for id: ', sessionResult.sid);
            console.log(err);
            res.sendStatus(202);
          }
      ...
      this._validate(body, function(err, user, attributes) {
        if (err) {
          ...
          if (this.single_logout) {
            ...
            req.sessionStore.set(req.query.ticket, {
              sid: req.session.id,
              cookie: req.session.cookie
            }, function(err) {
              /* istanbul ignore if */
              if (err) {
                console.info('Trying to store ticket in sessionStore for ssoff failed!');
                console.error(err);
              }
            });
          }
